...
const EditModal = ({ item }: any) => {
    ...
    const [receive, setReceive] = useState<boolean>(item.receive);
    const [installments, setInstallments] = useState<number>(item.installments.length);
    ...
    const closeModal = () => {
        return auth.editModalIsOpen({}, false);
    }
    ...
    const validateItem = () => {
        ...
        return sendNewItem(itemSend)
    }

    const sendNewItem = async (itemSend: object) => {
        try {
            ...
            const response: any = await api.items.updateItem(userId, itemSend)
            ...
            auth.responseMessage({ response: 'success', message: i18n.t('sucessModified') })
            return closeModal()
        } catch (e) {
            return e
        }
    ...
    return (
        <>
            ...
            <ModalComplet>
                ...
                <button onClick={validateItem}>{i18n.t('changeItem')}</button>
            </ModalComplet>
        </>
    );
};
